...
String coverPremium = "0.0";
	...
	if(!AP_RP_Flag.equals("") && !s_CoverName.equalsIgnoreCase("Other Types") && Double.parseDouble(coverPremium) != 0.0){
		...
		if(!CONFIG.getProperty("testEnvironment").equalsIgnoreCase("88")){
			//driver.findElement(By.xpath(Tech_AdjustXP)).click();
			
			//Click on appy book rate button
			//if(!(s_CoverName.replaceAll(" ", "").equalsIgnoreCase("Trailers"))){
		...
		//
		 if(abvr.equals("MFB_OTBS")){
			 ...
			 if(!CONFIG.getProperty("testEnvironment").equalsIgnoreCase("88")){
				 ...
				 if(s_CoverName.replaceAll(" ", "").equalsIgnoreCase("Trailers")){
					 rtf = Double.parseDouble(k.getAttributeByXpath(Tbl+"["+getTableHeaderIndex(tblH,"Rateable")+"]/input","value"));
				 }else{
					 rtf = Double.parseDouble(k.getTextByXpath(Tbl+"["+getTableHeaderIndex(tblH,"Rateable")+"]"));
				 }
			 }
...
public boolean AddSpecifiedTrailers(Map<Object, Object> map_data){
	...
	try{
		...
		while(no < ItemSize){
			...
			if(CONFIG.getProperty("testEnvironment").equalsIgnoreCase("88")){
				String speciXpath = "//*[@id='p170_p170_f2_h1']//following::div[@class='button'][1]//a";
				JavascriptExecutor j_exe = (JavascriptExecutor) driver;
				j_exe.executeScript("arguments[0].scrollIntoView(true);", driver.findElement(By.xpath(speciXpath)));
				
				//Click on Add item button.
				driver.findElement(By.xpath(speciXpath)).click();
			}else{
...
public boolean AddUnspecifiedTrailers(Map<Object, Object> map_data){
	...
	try{
		...
		while(no < ItemSize){
			...
			if(CONFIG.getProperty("testEnvironment").equalsIgnoreCase("88")){
				String speciXpath = "//*[@id='p170_p170_f3_h1']//following::div[@class='button'][1]//a";
				JavascriptExecutor j_exe = (JavascriptExecutor) driver;
				j_exe.executeScript("arguments[0].scrollIntoView(true);", driver.findElement(By.xpath(speciXpath)));
...
//				double ExpPremium, ActPremium;
		...
		public boolean PersonalAccidentOptionalPage(Map<Object, Object> map_data)
		{
			try
			{
				PoaNetP=0.00;
				String cur="";
				if(TestBase.product.equals("MFB")) cur="€";
				else if(TestBase.product.equals("MFC")) cur="£";
