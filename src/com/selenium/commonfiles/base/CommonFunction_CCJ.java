...
public class CommonFunction_CCJ extends TestBase{
	...
	public boolean addProperty(Map<Object, Object> map_data,int count){
		...
		try{
			...
			//Added below code to create 100 location policies
			/*if(locationBName.contains("Specified")){
...
public boolean renewalSearchPolicyNEW(Map<Object,Object> map_data){
	...
	if(rows.size()>0){
		...
		//Rewind button's visibility check
		if(((String)map_data.get("Renewal_Status")).contains("Rewind")) {
			if(!isRewindButtonPresent(StartDate)) {
				TestUtil.reportStatus("Rewind button is not present as difference between todays date <b> [ "+new SimpleDateFormat("dd/MM/yyyy").format(new Date())+" ] </b> and policy start date <b> [ "+StartDate+" ] </b> is more than rewind button duration <b>[ "+Integer.parseInt(CONFIG.getProperty("RewindButtonDurration"))+" ] </b> .Hence <b>[ "+map_data.get("Renewal_Status")+" ] </b> flow is not possible on selected policy . ", "Info", true);
				return false;	
			}
		}
...
/**
 * 
 * This methods decides whether rewind button is present or not based on policy start date
 */
public boolean isRewindButtonPresent(String policyStartDate) {
	...
	try {
		...
		System.out.println("*** Error while deciding rewind buttons visibility - "+e.getMessage());
		return false;
	}
	return true;
}
