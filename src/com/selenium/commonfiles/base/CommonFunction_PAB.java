...
//				double ExpPremium, ActPremium;
		...
		public boolean funcPersonalAccident(Map<Object, Object> mdata) 
		{
			...
			try 
			{
				...
				double c_mw_en = Double.parseDouble((String)mdata.get("PAA_Man_employee")); // Number of employee 
				double c_nmw_en = Double.parseDouble((String)mdata.get("PAA_Non_Man_employee"));
				
				double c_mw_rf = Double.parseDouble((String)mdata.get("PAA_Man_annual_wageroll")); // Rateable Factor
				double c_nmw_rf = Double.parseDouble((String)mdata.get("PAA_Non_Man_annual_wageroll"));
				
				double c_mw_br = (Manual_IR - (Manual_IR *(Discount/100))); //Book Rate   
				double c_nmw_br = (NonManual_IR - (NonManual_IR *(Discount/100)));
				
				double c_mw_bp = c_mw_rf * (c_mw_br / 100); // Book Premium
				double c_nmw_bp = c_nmw_rf * (c_nmw_br / 100);
				
				//Revised Premium
				double c_mw_rp = c_mw_bp +( c_mw_bp * (Double.parseDouble((String)mdata.get("PAA_Man_TechAdjust"))/100));
				double c_nmw_rp = c_nmw_bp +( c_nmw_bp * (Double.parseDouble((String)mdata.get("PAA_Non_Man_TechAdjust"))/100));
				...
				if(Double.parseDouble(k.getAttributeByXpath(PA_Ratetblxp+r1+mw+rc10, "value"))>0) c_mw_np = Double.parseDouble(k.getAttributeByXpath(PA_Ratetblxp+r1+mw+rc10, "value"));				
				if(Double.parseDouble(k.getAttributeByXpath(PA_Ratetblxp+r1+nmw+rc10, "value"))>0) c_nmw_np = Double.parseDouble(k.getAttributeByXpath(PA_Ratetblxp+r1+nmw+rc10, "value"));
				
				//Illness Benefit
				double c_ill_rateable = 0.00;
				double c_ill_ir = 0.00;
				double c_ill_br = 0.00;
				...
				if(((String) mdata.get("PAA_illness_benifit")).equalsIgnoreCase("Yes"))
				{
					...
					c_ill_ir = Double.parseDouble(OR.properties.getProperty("Illness_Initial_Rate"));
					c_ill_br = c_ill_ir;
					c_ill_bp = c_ill_rateable *(c_ill_ir / 100);
					c_ill_rp = c_ill_bp + c_ill_bp * (Double.parseDouble(k.getAttributeByXpath(PA_Ratetblxp+r1+ill+rc8,"value")) /100);
					
					double s_ill_po = Double.parseDouble(k.getAttributeByXpath(PA_Ratetblxp+r1+ill+rc10, "value"));
					if(s_ill_po>0) c_ill_np= s_ill_po;
					else c_ill_np = c_ill_rp;
				}
				
				// Premium
				double c_np_total = Double.parseDouble(f.format(c_mw_np + c_nmw_np + c_ill_np));
				...
				if(TestBase.businessEvent.equalsIgnoreCase("MTA") && ((String)common.MTA_excel_data_map.get("MTA_ExistingPolicy")).equalsIgnoreCase("Yes"))
				{
					if(aprp_PremiumOverride!=null)
					{
						...
						if(((String) mdata.get("PAA_illness_benifit")).equalsIgnoreCase("Yes"))
						{
							...
							c_ill_rateable = c_mw_np + c_nmw_np;
							c_ill_bp = Double.parseDouble(f.format(c_ill_rateable *(c_ill_ir / 100)));				
							c_ill_rp = c_ill_bp + c_ill_bp * (Double.parseDouble(k.getAttributeByXpath(PA_Ratetblxp+r1+ill+rc8,"value")) /100);
						}					
					}
				}
				//recalculate after AP/RP operation
				c_np_total = Double.parseDouble(f.format(c_mw_np + c_nmw_np + c_ill_np));
				
				//On Screen:
				
				Double s_mw_en = Double.parseDouble(k.getTextByXpath(PA_Ratetblxp+r1+mw+rc2));// Employees Number
				Double s_nmw_en = Double.parseDouble(k.getTextByXpath(PA_Ratetblxp+r1+nmw+rc2));
				...
				double s_mw_po = Double.parseDouble(k.getAttributeByXpath(PA_Ratetblxp+r1+mw+rc10, "value")); // Premium Override
				double s_nmw_po = Double.parseDouble(k.getAttributeByXpath(PA_Ratetblxp+r1+nmw+rc10, "value"));
				
				double s_ill_rf = 0.00; // Illness Benefit values
				double s_ill_ir = 0.00;
				double s_ill_aa = 0.00;
				double s_ill_br = 0.00;
				double s_ill_rp = 0.00;
				...
				if(((String) mdata.get("PAA_illness_benifit")).equalsIgnoreCase("Yes"))
				{
					...
					s_ill_po = Double.parseDouble(k.getAttributeByXpath(PA_Ratetblxp+r1+ill+rc10, "value"));
				}
				
				//Premium
				double s_mw_np = Double.parseDouble(k.getAttributeByXpath(PA_Ratetblxp+r1+mw+rc11, "value"));
				double s_nmw_np = Double.parseDouble(k.getAttributeByXpath(PA_Ratetblxp+r1+nmw+rc11, "value"));
				double s_ill_np = Double.parseDouble(k.getAttributeByXpath(PA_Ratetblxp+r1+ill+rc11, "value"));
				String s_np_total = k.getAttributeByXpath("//*[@id='pab_pa_tot']", "value");
				
				
				//Comparison
				
				// Employees Number
				common.compareValues(c_mw_en, s_mw_en, "Employee Number for Manual");
				common.compareValues(c_nmw_en, s_nmw_en, "Employee Number for Non Manual");
				
				// Employees Rateable factor
				common.compareValues(c_mw_rf, s_mw_rf, "Rateable factor for Manual");
				common.compareValues(c_nmw_rf, s_nmw_rf, "Rateable factor for Non Manual");
				
				//Initial Rate 
				common.compareValues(Manual_IR, s_mw_ir, "Initial Rate for Manual");
				common.compareValues(NonManual_IR, s_nmw_ir, "Initial Rate factor for Non Manual");
				
				//Auto Adjustment %
				common.compareValues(-Discount, s_mw_aa, "Auto Adjustment for Manual");
				common.compareValues(-Discount, s_nmw_aa, "Auto Adjustment factor for Non Manual");
				
				//Book Rate								
				common.compareValues(c_mw_br, s_mw_br, "Manual BookRate");
				common.compareValues(c_nmw_br, s_nmw_br, "Non Manual BookRate");
				
				//Book Premium
				common.compareValues(c_mw_bp, s_mw_bp, "Manual Book Premium");
				common.compareValues(c_nmw_bp, s_nmw_bp, "Non Manual Premium");
				
				customAssert.assertTrue(k.Click("PAA_applyBookrate"), "Unable to click on PAA_applyBookrate");
				k.waitTwoSeconds();
				
				// Revised Premium
				common.compareValues(c_mw_rp, s_mw_rp, "Revised Premium for Manual Work ");
				common.compareValues(c_nmw_rp, s_nmw_rp, "Revised Premium for Non Manual Work ");
				
				//Illness Benefit
				if(((String) mdata.get("PAA_illness_benifit")).equalsIgnoreCase("Yes"))
				{
					common.compareValues(c_ill_rateable, s_ill_rf, "Illness Rateable factor");
					common.compareValues(c_ill_ir, s_ill_ir,"Illness Initial Rate ");
					common.compareValues(0, s_ill_aa,"Illness Auto Adjustment ");
					common.compareValues(c_ill_br, s_ill_br,"Illness Book Rate ");
					common.compareValues(c_ill_rp, s_ill_rp,"Illness Revised Premium ");
				}
				
				//Premium
				common.compareValues(c_mw_np, s_mw_np, "Manual Work Premium");
				common.compareValues(c_nmw_np, s_nmw_np, "Non Manual Work Premium");
				common.compareValues(c_ill_np, s_ill_np, "Illness Premium");
				common.compareValues(c_np_total, Double.parseDouble(s_np_total), "Personal Accident Total Premium");	
			
				mdata.put("PS_PersonalAccident_NetNetPremium", s_np_total);
				
				if((Discount == 0.0) &&(c_np_total == 0.00) && (k.getAttribute("PAA_benefit_amount", "value").equals("Other")))
				{
